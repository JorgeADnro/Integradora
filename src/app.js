...
app.get('/temas', (req,res) =>{
    conexion.query('SELECT * FROM temas', (err,filas)=>{
        if(err){
            throw err;
        }else{
            res.send(filas);
...
//Mostrar uno de los cursos
app.get('/temas/:cveTema', (req,res) =>{
    conexion.query('SELECT * FROM temas WHERE cveTema = ?',[req.params.cveTema], (err,fila)=>{
        if(err){
            throw err;
        }else{
            res.send(fila);
...
app.get('/actividades', (req,res) =>{
    conexion.query('SELECT * FROM temas inner join area_conocimiento on temas.cveArea = area_conocimiento.cveArea where area_conocimiento.cveArea=2', (err,filas)=>{
        if(err){
            throw err;
        }else{
            res.send(filas);
